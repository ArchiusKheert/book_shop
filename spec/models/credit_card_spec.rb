...
RSpec.describe CreditCard, type: :model do
  ...
  it {expect(subject).to allow_value('05/22').for(:month_year) }
  it {expect(subject).to allow_value('12/24').for(:month_year)}
  it {expect(subject).not_to allow_value('00/21').for(:month_year) }
  it {expect(subject).not_to allow_value('13/21').for(:month_year) }
  ...
  it { expect(subject).to allow_value('Archius Kheert').for(:name_on_card) }
